...
Usage::
    ...
    python grid_detect.py                          # test.cas.h5, default measures
    python grid_detect.py path/to/case.cas.h5
    python grid_detect.py --check                  # Fluent-style mesh check
    python grid_detect.py --d test.cas.h5          # print only the dimension (2d/3d)
    python grid_detect.py --all                    # every measure valid for the mesh
    python grid_detect.py --measures skew aspect_ratio --csv quality.csv
    python grid_detect.py --hist quality.png --json quality.json
    python grid_detect.py --show                   # 3D view of cells outside range
"""
...
#: VTK ids of the cell types that enclose a volume (the 3D Fluent cells);
#: a mesh without any of these consists of planar cells, i.e. is 2D.
_3D_CELL_TYPE_IDS = [int(pv.CellType.TETRA), int(pv.CellType.PYRAMID),
                     int(pv.CellType.WEDGE), int(pv.CellType.HEXAHEDRON),
                     int(pv.CellType.POLYHEDRON)]

#: Node count in the legacy ``cells`` array per fixed-size VTK cell type
#: (every type Fluent writes except polyhedra). Any other type present
#: makes :func:`_connectivity_offsets` fall back to the per-cell loop.
_REGULAR_CELL_NODES = {
    int(pv.CellType.VERTEX): 1,
    int(pv.CellType.LINE): 2,
    int(pv.CellType.TRIANGLE): 3,
    int(pv.CellType.QUAD): 4,
    int(pv.CellType.TETRA): 4,
    int(pv.CellType.PYRAMID): 5,
    int(pv.CellType.WEDGE): 6,
    int(pv.CellType.HEXAHEDRON): 8,
}
...
def mesh_dimension(file_path: str) -> str:
    """Return the dimension of a Fluent case file as ``'2d'`` or ``'3d'``.

    The case is 3D when any cell zone contains 3D cell types (tetra,
    pyramid, wedge, hexahedron, polyhedron) and 2D when the cell zones
    hold only planar cells. Only the cell types are inspected — no volumes
    and no face geometry are computed — so this costs little more than
    reading the file, against the minutes a full quality evaluation needs.
    ...
    try:  # silence VTK notices such as "No data file (.dat.h5) found"
        ...
        pass
    _, blocks = load_mesh_blocks(file_path)
    for _, mesh in blocks:
        if np.any(np.isin(np.asarray(mesh.celltypes), _3D_CELL_TYPE_IDS)):
            return '3d'
    return '2d'
...
def _connectivity_offsets(mesh: pv.UnstructuredGrid):
    ...
    Notes
    -----
    Fixed-size cells (every Fluent type except polyhedra) are located with
    cumulative sums and verified against the array contents, so the
    per-cell Python loop only runs over the polyhedra themselves; the plain
    loop over all cells remains only as a last resort for unknown
    variable-size types.
    """
    faces = np.asarray(mesh.cells)
    n_cells = mesh.n_cells
    celltypes = np.asarray(mesh.celltypes)
    sizes = np.full(n_cells, -1, dtype=np.int64)
    for raw_type in np.unique(celltypes):
        n = _REGULAR_CELL_NODES.get(int(raw_type), -1)
        if n >= 0:
            sizes[celltypes == raw_type] = n
    if n_cells and np.all(sizes >= 0):  # only fixed-size cells: no loop
        starts = np.concatenate(([0], np.cumsum(sizes[:-1] + 1)))
        if int(starts[-1]) + int(sizes[-1]) + 1 == faces.size \
                and np.array_equal(faces[starts], sizes):
            return starts + 1, sizes, {}

    is_poly = celltypes == int(pv.CellType.POLYHEDRON)
    if n_cells and np.all(sizes[~is_poly] >= 0):  # fixed-size + polyhedra
        # A polyhedron's entry starts after the regular entries before it
        # (prefix ``reg_prefix``) plus the total length of the previous
        # polyhedra, so only the polyhedra need a sequential parse.
        reg_prefix = np.concatenate(
            ([0], np.cumsum(np.where(is_poly, 0, sizes + 1))))
        poly_idx = np.where(is_poly)[0]
        poly_lengths = np.empty(len(poly_idx), dtype=np.int64)
        id_starts = np.empty(n_cells, dtype=np.int64)
        n_nodes = np.where(is_poly, 0, sizes)
        poly_faces: dict[int, list[np.ndarray]] = {}
        length_sum = 0
        for k, i in enumerate(poly_idx):
            count_pos = int(reg_prefix[i]) + length_sum
            n_faces = -int(faces[count_pos])
            pos = count_pos + 1
            length = 1
            cell_faces = []
            for _ in range(n_faces):
                nodes = int(faces[pos])
                pos += 1
                cell_faces.append(faces[pos:pos + nodes])
                pos += nodes
                length += 1 + nodes
            id_starts[i] = count_pos + 1
            poly_lengths[k] = length
            poly_faces[int(i)] = cell_faces
            length_sum += length
        contribution = np.zeros(n_cells, dtype=np.int64)
        contribution[poly_idx] = poly_lengths
        poly_prefix = np.concatenate(([0], np.cumsum(contribution)))
        id_starts[~is_poly] = (reg_prefix[:-1] + poly_prefix[:-1])[~is_poly] + 1
        if int(reg_prefix[-1] + poly_prefix[-1]) == faces.size \
                and np.all(faces[id_starts[poly_idx] - 1] < 0):
            return id_starts, n_nodes, poly_faces

    # last resort: per-cell loop (unknown variable-size types present)
    id_starts = np.empty(n_cells, dtype=np.int64)
...
def _polygon_areas_and_centroids(ids: np.ndarray, counts: np.ndarray,
    ...
    # successor of every vertex (wrapping to the first vertex of the row);
    # a column shift is much faster than take_along_axis, and the wrap
    # column of short rows is fixed with one scattered assignment
    p_next = np.empty_like(coords)
    p_next[:, :-1] = coords[:, 1:]
    p_next[:, -1] = coords[:, 0]
    rows = np.arange(ids.shape[0])
    p_next[rows, np.maximum(counts - 1, 0)] = coords[rows, 0]
...
def _face_geometry(mesh: pv.UnstructuredGrid):
    ...
    group_ks: list[np.ndarray] = []
    poly_rows: list[np.ndarray] = []
    poly_lists: list[list[np.ndarray]] = []
    for raw_type in np.unique(celltypes):
        t = int(raw_type)
        rows = np.where(celltypes == raw_type)[0]
        if t == int(pv.CellType.POLYHEDRON):
            for i in rows:
                flist = poly_faces.get(int(i))
                if flist:
                    poly_rows.append(i)
                    poly_lists.append(flist)
            continue
        table = _FACES_2D.get(t, _FACES_3D.get(t))
        if table is None:
            continue  # unsupported type: owns no faces
        ftab = np.asarray(table)
        starts, ns = id_starts[rows], n_nodes[rows]
        if np.all(ns == ns[0]):  # fixed-size type: one vectorized gather
            node_ids = conn[starts[:, None] + np.arange(ns[0])]
        else:
            node_ids = np.stack([conn[s:s + n] for s, n in zip(starts, ns)])
        ...
        group_ks.append(np.full(len(rows) * len(table), ftab.shape[1],
                                dtype=np.int64))

    if poly_lists:  # every polyhedron face as one row of a padded matrix
        poly_kmax = max(max(len(f) for f in fl) for fl in poly_lists)
        n_pf = sum(len(fl) for fl in poly_lists)
        poly_mat = np.full((n_pf, poly_kmax), -1, dtype=np.int64)
        poly_ks = np.empty(n_pf, dtype=np.int64)
        poly_owner = np.empty(n_pf, dtype=np.int64)
        pos_p = 0
        for i, fl in zip(poly_rows, poly_lists):
            nf = len(fl)
            for j, f in enumerate(fl):
                poly_mat[pos_p + j, :len(f)] = f
                poly_ks[pos_p + j] = len(f)
            poly_owner[pos_p:pos_p + nf] = i
            pos_p += nf
        face_cells.append(poly_owner)
        face_ids.append(poly_mat)
        group_ks.append(poly_ks)
    ...
    pos = 0
    for cells_part, ids_part, ks_part in zip(face_cells, face_ids, group_ks):
        m = len(ids_part)
        face_nodes[pos:pos + m, :ids_part.shape[1]] = ids_part
        face_ks[pos:pos + m] = ks_part
...
def fluent_orthogonal(mesh: pv.UnstructuredGrid,
                      geometry: tuple | None = None) -> np.ndarray:
    ...
    geometry : tuple, optional
        Precomputed :func:`_face_geometry` result for ``mesh``; pass it when
        several measures run on the same mesh to avoid rebuilding it.
    ...
    if mesh.n_cells == 0:
        return np.zeros(0)
    if geometry is None:
        geometry = _face_geometry(mesh)
    face_nodes, face_ks, owner, areas, face_centroids, cell_centroids, _, _ = \
        geometry
    ...
    # Interior faces are matched per node-count group: sorting whole padded
    # rows would drag the -1 padding into the sort and mixes face sizes in
    # one wide lexicographic sort. The void view turns each row into one
    # sortable scalar, which is much faster than ``np.unique(axis=0)``.
    inverse = np.empty(face_nodes.shape[0], dtype=np.int64)
    n_groups = 0
    for k in np.unique(face_ks):
        rows = np.where(face_ks == k)[0]
        keys = np.sort(face_nodes[rows, :k], axis=1)
        key_view = np.ascontiguousarray(keys).view(
            np.dtype((np.void, keys.dtype.itemsize * k))).ravel()
        _, group = np.unique(key_view, return_inverse=True)
        inverse[rows] = group + n_groups
...
def fluent_aspect_ratio(mesh: pv.UnstructuredGrid,
                        geometry: tuple | None = None) -> np.ndarray:
...
def fluent_mesh_check(file_path: str, verbose: bool = True) -> dict:
    ...
    * ``maximum_aspect_ratio`` - Fluent definition, see
    ...
    A block is reported as 2D when it contains no 3D cell types, which for
    valid Fluent meshes matches the volume rule (no cell with positive
    volume) but costs nothing to evaluate; the per-cell volumes of
    ``compute_cell_sizes`` take minutes on multi-million-cell meshes.
    ...
    for name, mesh in blocks:
        # 2D/3D from the cell types (see the docstring): free, while the
        # volume rule would need compute_cell_sizes (~80 s for 2M cells).
        is_2d = not np.any(np.isin(np.asarray(mesh.celltypes),
                                   _3D_CELL_TYPE_IDS))

        # Fluent face-area statistics: the smallest face area in 3D, the
        # smallest edge length in 2D (there the "faces" are edges). The
        # face geometry is built once and shared with both measures below.
        geometry = _face_geometry(mesh)
        _, _, _, areas, _, _, _, _ = geometry
        face_area = np.linalg.norm(areas, axis=1)
        min_face = float(face_area.min()) if face_area.size else np.nan

        oq = fluent_orthogonal(mesh, geometry)
        ar = fluent_aspect_ratio(mesh, geometry)
...
def compute_quality(mesh: pv.DataSet, measures: list[str]) -> tuple[dict[str, np.ndarray], dict]:
    ...
    if custom:  # one shared face geometry serves all custom measures
        geometry = _face_geometry(mesh)
    for measure in custom:
        arrays[measure] = custom_functions[measure](mesh, geometry)
...
def main() -> None:
    ...
    parser.add_argument('--d', action='store_true',
                        help='print only the mesh dimension (2d or 3d) and exit')
    ...
    parser.add_argument('--csv', nargs='?', const='grid_detect.csv', default=None,
                        metavar='FILE', help='save per-cell values to CSV')
    parser.add_argument('--json', nargs='?', const='grid_detect.json', default=None,
                        metavar='FILE', help='save summary to JSON')
    parser.add_argument('--hist', nargs='?', const='grid_detect.png', default=None,
    ...
    if args.d:
        print(mesh_dimension(args.file_path))
        return

    if args.check:
        report = fluent_mesh_check(args.file_path)
        if args.json:  # only the per-block stats; the overall values are
            payload = {'file': report['file'], 'reader': report['reader'],
                       'blocks': report['blocks']}  # their min/max otherwise
